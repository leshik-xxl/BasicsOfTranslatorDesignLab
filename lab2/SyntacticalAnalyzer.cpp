...
SyntacticalAnalyzer::read_res SyntacticalAnalyzer::readKeyword(Node& n, const string& keyword)
{
   ...
   {
      string error = createError("'%s' keyword lexeme expected, but %s found", expected.c_str(),
...
SyntacticalAnalyzer::read_res SyntacticalAnalyzer::readDm(Node& n, const char delimiter)
{
   ...
   if (type != IdType::DM)
   {
      string error = createError("'%c' delimiter lexeme expected, but %s found", (char) delimiter,
                                 idTypeToString(type).c_str());
      read_res readRes;
      readRes.ok = false;
      readRes.error = error;
